...
class Score(object):
    ...
    def increase_current(self, reward):
        """Increase the the score of current game."""
...
def trainNetwork(s, readout, h_fc1, sess, debug):
    # initial score to keep track of the games' result
    score = Score(debug)
    ...
    while "flappy bird" != "angry bird":
        ...
        show_score(score, terminal, r_t)
        LOGGER.debug("TIMESTEP %s/ STATE %s/ EPSILON %s/ ACTION %s/ REWARD %s / Q_MAX %e",
...
def show_score(score, terminal, reward):
    if not terminal:
        if reward == 1:
            score.increase_current(reward)
    else:
        score.reset_current()
    score.show()
...
def playGame(debug):
    ...
    trainNetwork(s, readout, h_fc1, sess, debug)
